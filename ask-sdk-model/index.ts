export namespace services {
	...
	export abstract class BaseServiceClient {
		...
		private static buildUrl(
			...
			const processedEndpoint: string = endpoint.endsWith("/")
				? endpoint.substr(0, endpoint.length - 1)
				: endpoint;
			const pathWithParams: string = this.interpolateParams(
				path,
				pathParameters
			);
		...
		private requestInterceptors: Array<
			(request: ApiClientRequest) => void | Promise<void>
		> = [];
		private responseInterceptors: Array<
			(response: ApiClientResponse) => void | Promise<void>
		> = [];
		 ...
		 * @returns {BaseServiceClient}
		 */
		public withResponseInterceptors(
		...
		protected async invoke(
			...
			const request: ApiClientRequest = {
				url: BaseServiceClient.buildUrl(
					endpoint,
					path,
					queryParams,
					pathParams
			...
			try {
				...
				throw new SyntaxError(
					`Failed trying to parse the response body: ${response.body}`
				);
			}
	...
	export class LwaServiceClient extends BaseServiceClient {
		...
		constructor(options: {
			...
			if (options.authenticationConfiguration == null) {
				throw new Error(
					"AuthenticationConfiguration cannot be null or undefined."
				);
			}
			this.grantType = options.grantType
				? options.grantType
				: LwaServiceClient.CLIENT_CREDENTIALS_GRANT_TYPE;
		...
		public async getAccessToken(scope?: string): Promise<string> {
			const cacheKey: string = scope
				? scope
				: LwaServiceClient.REFRESH_ACCESS_TOKEN;
			const accessToken = this.tokenStore[cacheKey];

			if (
				accessToken &&
				accessToken.expiry > Date.now() + LwaServiceClient.EXPIRY_OFFSET_MILLIS
			) {
		...
		protected async generateAccessToken(
			...
			const authEndpoint =
				this.authenticationConfiguration.authEndpoint ||
				LwaServiceClient.AUTH_ENDPOINT;

			if (accessTokenRequest == null) {
				throw new Error(
					`Required parameter accessTokenRequest was null or undefined when calling generateAccessToken.`
				);
			}
			...
			const paramInfo =
				this.grantType === LwaServiceClient.LWA_CREDENTIALS_GRANT_TYPE
					? `&refresh_token=${accessTokenRequest.refreshToken}`
					: `&scope=${accessTokenRequest.scope}`;
			const bodyParams: string =
				`grant_type=${this.grantType}&client_secret=${accessTokenRequest.clientSecret}&client_id=${accessTokenRequest.clientId}` +
				paramInfo;
...
export function createUserAgent(
	...
	return (
		`ask-node-model/${packageVersion} Node/${process.version}` +
 ...
 * @enum
 */
export type SessionEndedErrorType =
	| "INVALID_RESPONSE"
...
export namespace dialog {
	 ...
	 * @interface
	 */
	export type UpdatedRequest =
		| dialog.UpdatedInputRequest
		| dialog.UpdatedIntentRequest;
}
...
export namespace interfaces.alexa.presentation {
	 ...
	 * @interface
	 */
	export type PresentationStateContext =
...
export namespace interfaces.alexa.presentation.apl {
	 ...
	 * @enum
	 */
	export type ComponentVisibleOnScreenMediaTagStateEnum =
		| "idle"
		| "playing"
		| "paused";
}
...
export namespace interfaces.alexa.presentation.apl {
	 ...
	 * @enum
	 */
	export type ComponentVisibleOnScreenScrollableTagDirectionEnum =
		| "horizontal"
		| "vertical";
}
...
export namespace interfaces.alexa.presentation.apl {
	 ...
	 * @enum
	 */
	export type ListRuntimeErrorReason =
...
export namespace interfaces.alexa.presentation.apla {
	 ...
	 * @enum
	 */
	export type AudioSourceErrorReason =
		| "UNKNOWN_ERROR"
		| "INTERNAL_SERVER_ERROR"
		| "NOT_FOUND_ERROR"
		| "SSL_HANDSHAKE_ERROR"
...
export namespace interfaces.amazonpay.model.request {
	 ...
	 * @interface
	 */
	export type BaseAmazonPayEntity =
...
export namespace interfaces.display {
	...
	export type TextField =
		| interfaces.display.RichText
		| interfaces.display.PlainText;
}
...
export namespace interfaces.viewport {
	 ...
	 * @interface
	 */
	export type TypedViewportState =
...
export namespace services.datastore.v1 {
	 ...
	 * @enum
	 */
	export type CancelCommandsRequestErrorType =
		| "COMMANDS_DELIVERED"
		| "CONCURRENCY_ERROR"
		| "NOT_FOUND"
...
export namespace services.datastore.v1 {
	 ...
	 * @enum
	 */
	export type CommandsRequestErrorType =
...
export namespace services.datastore.v1 {
	 ...
	 * @enum
	 */
	export type QueuedResultRequestErrorType =
		| "NOT_FOUND"
		| "INVALID_REQUEST"
		| "INVALID_ACCESS_TOKEN"
		| "DATASTORE_SUPPORT_REQUIRED"
...
export namespace services.datastore.v1 {
	 ...
	 */
	export type Target =
		| services.datastore.v1.User
		| services.datastore.v1.Devices;
}
	...
	export class DatastoreServiceClient extends BaseServiceClient {
		...
		async callCommandsV1(
			...
			if (commandsRequest == null) {
				throw new Error(
					`Required parameter commandsRequest was null or undefined when calling ${__operationId__}.`
				);
			}
			...
			if (
				!headerParams.find(
					(param) => param.key.toLowerCase() === "content-type"
				)
			) {
			...
			const accessToken: string =
				await this.lwaServiceClient.getAccessTokenForScope("alexa::datastore");
			...
			const errorDefinitions: Map<number, string> = new Map<number, string>();
			errorDefinitions.set(
				200,
				"Multiple CommandsDispatchResults in response."
			);
			errorDefinitions.set(400, "Request validation fails.");
			errorDefinitions.set(401, "Not Authorized.");
			errorDefinitions.set(
				403,
				"The skill is not allowed to execute commands."
			);
			errorDefinitions.set(
				429,
				"The client has made more calls than the allowed limit."
			);
		...
		async commandsV1(
			...
			const apiResponse: ApiResponse = await this.callCommandsV1(
				commandsRequest
			);
		...
		async callCancelCommandsV1(queuedResultId: string): Promise<ApiResponse> {
			...
			if (queuedResultId == null) {
				throw new Error(
					`Required parameter queuedResultId was null or undefined when calling ${__operationId__}.`
				);
			}
			...
			errorDefinitions.set(
				403,
				"The skill is not allowed to call this API commands."
			);
		...
		async callQueuedResultV1(
			...
			const queryParams: Array<{ key: string; value: string }> = [];
			if (maxResults != null) {
				const maxResultsValues: any[] = Array.isArray(maxResults)
					? maxResults
					: [maxResults];
				maxResultsValues.forEach((val) =>
					queryParams.push({ key: "maxResults", value: val!.toString() })
				);
			}
			if (nextToken != null) {
				const nextTokenValues: any[] = Array.isArray(nextToken)
					? nextToken
					: [nextToken];
				nextTokenValues.forEach((val) =>
					queryParams.push({ key: "nextToken", value: val })
				);
			}
			...
			errorDefinitions.set(
				200,
				"Unordered array of CommandsDispatchResult and pagination details."
			);
			errorDefinitions.set(400, "Request validation fails.");
			errorDefinitions.set(401, "Not Authorized.");
			errorDefinitions.set(
		 ...
		 * @param {string} nextToken The value of nextToken in the response to fetch next page. If not specified, the request fetches result for the first page.
		 */
		async queuedResultV1(
			queuedResultId: string,
			maxResults?: number,
			nextToken?: string
		): Promise<services.datastore.v1.QueuedResultResponse> {
			const apiResponse: ApiResponse = await this.callQueuedResultV1(
				queuedResultId,
				maxResults,
				nextToken
			);
...
export namespace services.deviceAddress {
	...
	export class DeviceAddressServiceClient extends BaseServiceClient {
		...
		async callGetCountryAndPostalCode(deviceId: string): Promise<ApiResponse> {
			...
			if (deviceId == null) {
				throw new Error(
					`Required parameter deviceId was null or undefined when calling ${__operationId__}.`
				);
			}
			...
			const authorizationValue =
				"Bearer " + this.apiConfiguration.authorizationValue;
			headerParams.push({ key: "Authorization", value: authorizationValue });
		 ...
		 * @param {string} deviceId The device Id for which to get the country and postal code
		 */
		async getCountryAndPostalCode(
			deviceId: string
		): Promise<services.deviceAddress.ShortAddress> {
			const apiResponse: ApiResponse = await this.callGetCountryAndPostalCode(
				deviceId
			);
		...
		async getFullAddress(
			deviceId: string
		): Promise<services.deviceAddress.Address> {
	...
	export class DirectiveServiceClient extends BaseServiceClient {
		...
		async callEnqueue(
			...
			if (sendDirectiveRequest == null) {
				throw new Error(
					`Required parameter sendDirectiveRequest was null or undefined when calling ${__operationId__}.`
				);
			}
			...
			errorDefinitions.set(
				403,
				"The skill is not allowed to send directives at the moment."
			);
			errorDefinitions.set(0, "Unexpected error.");
...
export namespace services.listManagement {
	...
	export class ListManagementServiceClient extends BaseServiceClient {
		...
		async callDeleteList(listId: string): Promise<ApiResponse> {
			...
			if (listId == null) {
				throw new Error(
					`Required parameter listId was null or undefined when calling ${__operationId__}.`
				);
			}
		...
		async callDeleteListItem(
			...
			if (itemId == null) {
				throw new Error(
					`Required parameter itemId was null or undefined when calling ${__operationId__}.`
				);
			}
		...
		async getListItem(
			...
			const apiResponse: ApiResponse = await this.callGetListItem(
				listId,
				itemId
			);
		...
		async callUpdateListItem(
			...
			if (updateListItemRequest == null) {
				throw new Error(
					`Required parameter updateListItemRequest was null or undefined when calling ${__operationId__}.`
				);
			}
		...
		async updateListItem(
			...
			const apiResponse: ApiResponse = await this.callUpdateListItem(
				listId,
				itemId,
				updateListItemRequest
			);
		...
		async callCreateListItem(
			...
			if (createListItemRequest == null) {
				throw new Error(
					`Required parameter createListItemRequest was null or undefined when calling ${__operationId__}.`
				);
			}
		...
		async createListItem(
			...
			const apiResponse: ApiResponse = await this.callCreateListItem(
				listId,
				createListItemRequest
			);
			return apiResponse.body as services.listManagement.AlexaListItem;
		...
		async callUpdateList(
			...
			if (updateListRequest == null) {
				throw new Error(
					`Required parameter updateListRequest was null or undefined when calling ${__operationId__}.`
				);
			}
		...
		async updateList(
			...
			const apiResponse: ApiResponse = await this.callUpdateList(
				listId,
				updateListRequest
			);
		...
		async callGetList(listId: string, status: string): Promise<ApiResponse> {
			...
			if (status == null) {
				throw new Error(
					`Required parameter status was null or undefined when calling ${__operationId__}.`
				);
			}
		...
		async callCreateList(
			...
			if (createListRequest == null) {
				throw new Error(
					`Required parameter createListRequest was null or undefined when calling ${__operationId__}.`
				);
			}
		...
		async createList(
			...
			const apiResponse: ApiResponse = await this.callCreateList(
				createListRequest
			);
			return apiResponse.body as services.listManagement.AlexaListMetadata;
		}
	...
	export class MonetizationServiceClient extends BaseServiceClient {
		...
		async callGetInSkillProducts(
			...
			if (acceptLanguage == null) {
				throw new Error(
					`Required parameter acceptLanguage was null or undefined when calling ${__operationId__}.`
				);
			}

			const queryParams: Array<{ key: string; value: string }> = [];
			if (purchasable != null) {
				const purchasableValues: any[] = Array.isArray(purchasable)
					? purchasable
					: [purchasable];
				purchasableValues.forEach((val) =>
					queryParams.push({ key: "purchasable", value: val })
				);
			}
			if (entitled != null) {
				const entitledValues: any[] = Array.isArray(entitled)
					? entitled
					: [entitled];
				entitledValues.forEach((val) =>
					queryParams.push({ key: "entitled", value: val })
				);
			}
			if (productType != null) {
				const productTypeValues: any[] = Array.isArray(productType)
					? productType
					: [productType];
				productTypeValues.forEach((val) =>
					queryParams.push({ key: "productType", value: val })
				);
			}
			if (nextToken != null) {
				const nextTokenValues: any[] = Array.isArray(nextToken)
			...
			errorDefinitions.set(
				200,
				"Returns a list of In-Skill products on success."
			);
		...
		async getInSkillProducts(
			...
			const apiResponse: ApiResponse = await this.callGetInSkillProducts(
				acceptLanguage,
				purchasable,
				entitled,
				productType,
		...
		async callGetInSkillProduct(
			...
			if (productId == null) {
				throw new Error(
					`Required parameter productId was null or undefined when calling ${__operationId__}.`
				);
			}
			...
			let resourcePath: string =
				"/v1/users/~current/skills/~current/inSkillProducts/{productId}";
		 ...
		 * @param {string} productId Product Id.
		 */
		async getInSkillProduct(
			acceptLanguage: string,
			productId: string
		): Promise<services.monetization.InSkillProduct> {
			const apiResponse: ApiResponse = await this.callGetInSkillProduct(
				acceptLanguage,
				productId
			);
		 ...
		 */
		async callGetInSkillProductsTransactions(
			acceptLanguage: string,
			productId?: string,
			status?: string,
			...
			const queryParams: Array<{ key: string; value: string }> = [];
			if (productId != null) {
				const productIdValues: any[] = Array.isArray(productId)
					? productId
					: [productId];
				productIdValues.forEach((val) =>
					queryParams.push({ key: "productId", value: val })
				);
			}
			if (status != null) {
				const statusValues: any[] = Array.isArray(status) ? status : [status];
				statusValues.forEach((val) =>
					queryParams.push({ key: "status", value: val })
				);
			}
			if (fromLastModifiedTime != null) {
				const fromLastModifiedTimeValues: any[] = Array.isArray(
					fromLastModifiedTime
				)
					? fromLastModifiedTime
					: [fromLastModifiedTime];
				fromLastModifiedTimeValues.forEach((val) =>
					queryParams.push({
						key: "fromLastModifiedTime",
						value: val!.toString(),
					})
				);
			}
			if (toLastModifiedTime != null) {
				const toLastModifiedTimeValues: any[] = Array.isArray(
					toLastModifiedTime
				)
					? toLastModifiedTime
					: [toLastModifiedTime];
				toLastModifiedTimeValues.forEach((val) =>
					queryParams.push({
						key: "toLastModifiedTime",
						value: val!.toString(),
					})
				);
			}
			...
			let resourcePath: string =
				"/v1/users/~current/skills/~current/inSkillProductsTransactions";
			...
			errorDefinitions.set(
				404,
				"Product id doesn&#39;t exist / invalid / not found."
			);
		 ...
		 * @param {number} maxResults sets the maximum number of results returned in the response body. If you want to retrieve fewer than upper limit of 100 results, you can add this parameter to your request. maxResults should not exceed the upper limit. The response might contain fewer results than maxResults, but it will never contain more. If there are additional results that satisfy the search criteria, but these results were not returned because maxResults was exceeded, the response contains nextToken which can be used to fetch next set of result.
		 */
		async getInSkillProductsTransactions(
			acceptLanguage: string,
			productId?: string,
			status?: string,
			fromLastModifiedTime?: string,
			...
			const apiResponse: ApiResponse =
				await this.callGetInSkillProductsTransactions(
					acceptLanguage,
					productId,
					status,
					fromLastModifiedTime,
		...
		async callGetVoicePurchaseSetting(): Promise<ApiResponse> {
			...
			let resourcePath: string =
				"/v1/users/~current/skills/~current/settings/voicePurchasing.enabled";
...
export namespace services.proactiveEvents {
	...
	export class ProactiveEventsServiceClient extends BaseServiceClient {
		 ...
		 */
		async callCreateProactiveEvent(
			...
			if (createProactiveEventRequest == null) {
				throw new Error(
					`Required parameter createProactiveEventRequest was null or undefined when calling ${__operationId__}.`
				);
			}
			...
			const accessToken: string =
				await this.lwaServiceClient.getAccessTokenForScope(
...
export namespace services.reminderManagement {
	...
	export class ReminderManagementServiceClient extends BaseServiceClient {
		...
		async callDeleteReminder(alertToken: string): Promise<ApiResponse> {
			...
			if (alertToken == null) {
				throw new Error(
					`Required parameter alertToken was null or undefined when calling ${__operationId__}.`
				);
			}
			...
			errorDefinitions.set(
				401,
				"UserAuthenticationException. Request is not authorized/authenticated e.g. If customer does not have permission to create a reminder."
			);
			errorDefinitions.set(
				429,
				"RateExceededException e.g. When the skill is throttled for exceeding the max rate"
			);
			errorDefinitions.set(500, "Internal Server Error");
		...
		async callUpdateReminder(
			...
			if (reminderRequest == null) {
				throw new Error(
					`Required parameter reminderRequest was null or undefined when calling ${__operationId__}.`
				);
			}
			...
			errorDefinitions.set(
				404,
				"NotFoundException e.g. Retured when reminder is not found"
			);
			errorDefinitions.set(
				409,
				"UserAuthenticationException. Request is not authorized/authenticated e.g. If customer does not have permission to create a reminder."
		...
		async updateReminder(
			...
			const apiResponse: ApiResponse = await this.callUpdateReminder(
				alertToken,
				reminderRequest
			);
		...
		async createReminder(
			...
			const apiResponse: ApiResponse = await this.callCreateReminder(
				reminderRequest
			);
			return apiResponse.body as services.reminderManagement.ReminderResponse;
		}
...
export namespace services.skillMessaging {
	...
	export class SkillMessagingServiceClient extends BaseServiceClient {
		...
		async callSendSkillMessage(
			...
			if (userId == null) {
				throw new Error(
					`Required parameter userId was null or undefined when calling ${__operationId__}.`
				);
			}
			// verify required parameter 'sendSkillMessagingRequest' is not null or undefined
			if (sendSkillMessagingRequest == null) {
				throw new Error(
					`Required parameter sendSkillMessagingRequest was null or undefined when calling ${__operationId__}.`
				);
			}
...
export namespace services.timerManagement {
	...
	export class TimerManagementServiceClient extends BaseServiceClient {
		...
		async callDeleteTimer(id: string): Promise<ApiResponse> {
			...
			if (id == null) {
				throw new Error(
					`Required parameter id was null or undefined when calling ${__operationId__}.`
				);
			}
		...
		async callCreateTimer(
			...
			if (timerRequest == null) {
				throw new Error(
					`Required parameter timerRequest was null or undefined when calling ${__operationId__}.`
				);
			}
...
export namespace services.ups {
	...
	export class UpsServiceClient extends BaseServiceClient {
		...
		async callGetProfileEmail(): Promise<ApiResponse> {
			...
			errorDefinitions.set(
				200,
				"Successfully retrieved the requested information."
			);
			...
			errorDefinitions.set(
				403,
				"The authentication token does not have access to resource."
			);
			errorDefinitions.set(
				429,
		 ...
		 * @param {string} deviceId The device Id
		 */
		async getSystemDistanceUnits(
			deviceId: string
		): Promise<services.ups.DistanceUnits> {
			const apiResponse: ApiResponse = await this.callGetSystemDistanceUnits(
				deviceId
			);
		 ...
		 * @param {string} deviceId The device Id
		 */
		async getSystemTemperatureUnit(
			deviceId: string
		): Promise<services.ups.TemperatureUnit> {
			const apiResponse: ApiResponse = await this.callGetSystemTemperatureUnit(
				deviceId
			);
		...
		async getSystemTimeZone(deviceId: string): Promise<string> {
			const apiResponse: ApiResponse = await this.callGetSystemTimeZone(
				deviceId
			);
			return apiResponse.body as string;
		}
		...
		async getPersonsProfileGivenName(): Promise<string> {
			const apiResponse: ApiResponse =
				await this.callGetPersonsProfileGivenName();
			return apiResponse.body as string;
		}
		/**
		...
		async getPersonsProfileMobileNumber(): Promise<services.ups.PhoneNumber> {
			const apiResponse: ApiResponse =
				await this.callGetPersonsProfileMobileNumber();
			return apiResponse.body as services.ups.PhoneNumber;
		}
		/**
...
export namespace services {
	...
	export class ServiceClientFactory {
		...
		getDeviceAddressServiceClient(): deviceAddress.DeviceAddressServiceClient {
			try {
				return new deviceAddress.DeviceAddressServiceClient(
					this.apiConfiguration
				);
			} catch (e) {
				const factoryError = new Error(
					`ServiceClientFactory Error while initializing DeviceAddressServiceClient: ${e.message}`
				);
		...
		getDirectiveServiceClient(): directive.DirectiveServiceClient {
			try {
				...
				const factoryError = new Error(
					`ServiceClientFactory Error while initializing DirectiveServiceClient: ${e.message}`
				);
				factoryError["name"] = "ServiceClientFactoryError";
		...
		getEndpointEnumerationServiceClient(): endpointEnumeration.EndpointEnumerationServiceClient {
			try {
				return new endpointEnumeration.EndpointEnumerationServiceClient(
					this.apiConfiguration
				);
			} catch (e) {
				const factoryError = new Error(
					`ServiceClientFactory Error while initializing EndpointEnumerationServiceClient: ${e.message}`
				);
				factoryError["name"] = "ServiceClientFactoryError";
		...
		getListManagementServiceClient(): listManagement.ListManagementServiceClient {
			try {
				return new listManagement.ListManagementServiceClient(
					this.apiConfiguration
				);
			} catch (e) {
				const factoryError = new Error(
					`ServiceClientFactory Error while initializing ListManagementServiceClient: ${e.message}`
				);
				factoryError["name"] = "ServiceClientFactoryError";
		...
		getMonetizationServiceClient(): monetization.MonetizationServiceClient {
			try {
				return new monetization.MonetizationServiceClient(
					this.apiConfiguration
				);
			} catch (e) {
				const factoryError = new Error(
					`ServiceClientFactory Error while initializing MonetizationServiceClient: ${e.message}`
				);
				factoryError["name"] = "ServiceClientFactoryError";
		...
		getReminderManagementServiceClient(): reminderManagement.ReminderManagementServiceClient {
			try {
				return new reminderManagement.ReminderManagementServiceClient(
					this.apiConfiguration
				);
			} catch (e) {
				const factoryError = new Error(
					`ServiceClientFactory Error while initializing ReminderManagementServiceClient: ${e.message}`
				);
				factoryError["name"] = "ServiceClientFactoryError";
		...
		getTimerManagementServiceClient(): timerManagement.TimerManagementServiceClient {
			try {
				return new timerManagement.TimerManagementServiceClient(
					this.apiConfiguration
				);
			} catch (e) {
				const factoryError = new Error(
					`ServiceClientFactory Error while initializing TimerManagementServiceClient: ${e.message}`
				);
				factoryError["name"] = "ServiceClientFactoryError";
		...
		getUpsServiceClient(): ups.UpsServiceClient {
			try {
				...
				const factoryError = new Error(
					`ServiceClientFactory Error while initializing UpsServiceClient: ${e.message}`
				);
				factoryError["name"] = "ServiceClientFactoryError";
